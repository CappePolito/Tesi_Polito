...
corner interpolation (Section~\ref{subsec:feature_detection}), pose estimation
(Section~\ref{subsec:pose_est_pipeline}), and hand-eye calibration
(Section~\ref{sec:hec_theory}).
...
PnP formulation described in Section~\ref{subsec:pose_est_pipeline}. The
...
threshold $\epsilon_{\max}$ defined in equation~\eqref{eq:reproj_error};
...
introduced in Section~\ref{sec:validation}, and the quantitative
...
greater sensitivity to noise in the estimated target poses. A detailed
quantitative comparison is presented in Chapter~\ref{sec:results}.
```
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		validation metrics (Section~\ref{sec:validation})
		\EndFor
...
While outlier filtering (Section~\ref{subsec:data_filtering}) removes
